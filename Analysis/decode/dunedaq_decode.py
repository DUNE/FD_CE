...
def wib_dec_onetrigger(triggerdata, fembs=range(4), fastchk=False, cd0cd1sync=True):
    tmts = [[],[],[],[],[],[],[],[]]
    cd_tmts = [[],[],[],[],[],[],[],[]]
    femb00 = []
    femb01 = []
    ...
    raw  = triggerdata
    bufs = [bytearray(np.array(raw['buff0']).tobytes()), bytearray(np.array(raw['buff1']).tobytes())]
    print(np.array(raw['buf_end_addrs']))
    buf_end_addr = np.array(raw['buf_end_addrs'])
    spy_rec_ticks = np.array(raw['spy_rec_ticks'])
    trig_cmd      = np.array(raw['trig_cmd'])
    if trig_cmd == 0:
        trigmode="SW"
    else:
        trigmode="HW"
    ...
    #wibdata.append([femb0, femb1, femb2, femb3, t0max])
    #wibdata.append([femb0, femb1, femb2, femb3, tmts, cd_tmts]) #for rado LArASIC analysis
    return [femb0, femb1, femb2, femb3,  t0max, tmts, cd_tmts] # for normal data collection operation 